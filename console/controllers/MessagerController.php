...
use common\commands\command\SendSMSCommand;
...
//            // acknowledge message
    ...
    public function actionRessiveSms($key, $phone, $body)
    {
        ...
        $model = \console\models\Task::find()->where(['key' => $key])->one();
        if ($model) {
    ...
    public function actionRessiveSocket($key, $message, $user)
    {
        ...
        $localsocket ="tcp://infomarketstudio.ru:1235";
...
//
//        Yii::error('handle socket errno: '.print_r($errno,true),'chat');
//        Yii::error('handle socket errstr: '.print_r($errstr,true),'chat');
